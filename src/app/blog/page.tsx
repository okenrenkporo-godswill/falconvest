          ...
          <div className="flex flex-col md:flex-row items-end justify-between gap-12">
            <div className="max-w-4xl">
              <motion.div
                ...
                className="px-4 py-1.5 rounded-full bg-[#01C1D6]/10 text-[#01C1D6] text-[10px] font-black uppercase tracking-[0.2em] w-fit mb-6"
              >
            ...
            <div className="flex gap-4">
               {["All", "Insights", "Guides", "News"].map((cat, i) => (
                 <button key={i} className={`px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${i === 0 ? "bg-[#01C1D6] text-white shadow-lg shadow-[#01C1D6]/20" : "bg-black/5 dark:bg-white/5 text-neutral-500 hover:text-black dark:hover:text-white"}`}>
                    {cat}
                 </button>
               ))}
            </div>
                ...
                >
                  ...
                  <div className="space-y-4">
                     <div className="flex items-center justify-between">
                        <span className="text-[#01C1D6] text-[10px] font-black uppercase tracking-[0.2em]">{post.category}</span>
                        <div className="flex items-center gap-3 text-neutral-400 text-xs font-medium">
                           <span className="flex items-center gap-1.5"><Calendar size={14} /> {post.date}</span>
                           <span className="flex items-center gap-1.5"><Clock size={14} /> {post.readTime}</span>
                        </div>
                     </div>
                     <h3 className="text-2xl font-black text-black dark:text-white uppercase tracking-tight group-hover:text-[#01C1D6] transition-colors leading-snug">{post.title}</h3>
