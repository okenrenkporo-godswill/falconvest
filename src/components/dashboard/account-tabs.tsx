...
export function AccountTabs({ totalBalance, balances }: AccountTabsProps) {
    ...
    return (
        <>
                ...
                <CardBody className="p-0">
                    ...
                    {activeTab === 'asset' && (
                        <div className="p-4 sm:p-6 space-y-6">
                            {hasAssets && balances && balances.length > 0 && (
                                            ...
                                            <div className="space-y-2">
                                                {balances.map((balance) => (
                                                    <div key={balance.id} className="flex items-center justify-between p-2 sm:p-3 rounded-lg hover:bg-default-50 transition-colors">
                                                        ...
                                                        <div className="text-right">
                                                            <p className="text-sm sm:text-base font-semibold">${(balance.usd_value ?? Number(balance.amount)).toFixed(2)}</p>
            ...
            {balances && <SwapModal isOpen={isSwapModalOpen} onOpenChange={() => setIsSwapModalOpen(false)} balances={balances} />}
        </>
    );
}
