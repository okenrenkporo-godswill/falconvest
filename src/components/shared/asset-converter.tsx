...
export function AssetConverter({ targetAsset, accountType, otherBalances, onConversionComplete }: AssetConverterProps) {
  ...
  useEffect(() => {
    if (swapFrom.size > 0) {
      ...
      fetchConversionRate(asset);
    }
  }, [swapFrom]);

  const fetchConversionRate = async (fromAsset: string) => {
    setLoadingRate(true);
    
    try {
      const fromCoinId = COINGECKO_IDS[fromAsset] || fromAsset.toLowerCase();
      const toCoinId = COINGECKO_IDS[targetAsset] || targetAsset.toLowerCase();
      ...
      const fromRate = data[fromCoinId]?.usd || 1;
      const toRate = data[toCoinId]?.usd || 1;
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      addToast({
        title: "Success",
        description: `Converted ${amount} ${selectedAsset} to ${targetAmount.toFixed(6)} ${targetAsset}`,
        color: "success",
      });
  ...
  return (
    <div className="bg-default-100 dark:bg-default-50/10 p-4 rounded-lg space-y-3">
        ...
        <div className="bg-default-200/50 dark:bg-default-100/10 p-2 rounded text-sm">
          <div className="flex justify-between">
            ...
            <span className="font-semibold">
              {loadingRate ? "Loading..." : `1 ${selectedAsset} = ${conversionRate.toFixed(6)} ${targetAsset}`}
            </span>
          </div>
          <div className="flex justify-between mt-1">
            <span className="text-default-500">You'll receive:</span>
            <span className="font-bold text-success">≈ {estimatedOutput} {targetAsset}</span>
          </div>
        </div>
      )}
      ...
      <Button
        ...
        isDisabled={!selectedAsset || !swapAmount || loadingRate}
